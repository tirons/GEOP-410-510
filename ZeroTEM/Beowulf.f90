!   PROGRAM Beowulf       Version 1.0.3    7 November 2007       Language: ANSI Standard Fortran 95
!   ======= =======       =============    ===============       ----------------------------------
!
!
...
!      14       NW1      Beowulf.mv1    Inversion plotting output
...
!          ------------------
!
!** RECORD 11.1: RES(1), RMU(1), REPS(1), CHRG(1), CTAU(1), CFREQ(1)
!** RECORD 11.2: RES(2), RMU(2), REPS(2), CHRG(2), CTAU(2), CFREQ(2)
!     .
!
!** RECORD 11.N: RES(N), RMU(N), REPS(N), CHRG(N), CTAU(N), CFREQ(N)
!
!           N = NLITH
...
! -----------------------------
 ...
 INTEGER, PARAMETER :: NPROP=6, QL=SELECTED_REAL_KIND(12,80)
 REAL, PARAMETER :: PI=3.141592654, PI2=PI/2., R2D=180./PI,  D2R=PI/180.
 INTEGER FVN,NR,NW,ND,NLG,NRI,NW1,MD1,MSG,MXERR,TDFD,STEP,NSX,ISTOP,KRXW, &
 ...
 DATA NR,NW,ND,NLG,NRI,NW1,NR1 /3,4,7,9,13,14,15/
 DATA MONTH /'JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'/
 DATA PVC, FVN /'Beowulf - Version 1.0.3   7 November 2007',100/
...
!  Initialise lithology list.
 ...
 LYTH(1:NLITH, 2) = 1.    !  Relative magnetic permeabilities
 LYTH(1:NLITH, 3) = 1.    !  Relative dielectric constants
 LYTH(1:NLITH, 4) = 0.    !  Chargeabilities
 LYTH(1:NLITH, 5) = 0.    !  CTAUs
 LYTH(1:NLITH, 6) = 1.    !  CFREQs
 ...
 DO J = 1,NLITH
   ...
   IF (LYTH(J,5) < 1.E-3 .OR. LYTH(J,6) < 1.E-15 .OR. LYTH(J,7) < 1.E-6)  THEN
     LYTH(J,4) = 0   ! default CHRG
     LYTH(J,5) = 0   ! default CTAU
     LYTH(J,6) = 1   ! default CFRQ
   END IF
 ...
 2 FORMAT(//T27,'LITHOLOGY PROPERTIES'/T27,'--------------------' &
          //T23,'Relative   Relative     Cole-Cole Parameters'    &
           /T9,'Resistivity      MU      Dielectric    CHRG      CTAU   CFREQ'/)
...
!  HEADER_ID is Line dependent for SURVEY_TYPE = 1
 ...
 WRITE(NR1,1) FVN,PVC,TRIM(TITLE)
 WRITE(NR1,20)
 ...
 WRITE(NW1,10) NLYR
 WRITE(NW1,11) RES_MOD(1:NLYR),THK_MOD(1:NLYR-1)
...
!==================================================================================================
 ...
 END MODULE INPUT_DATA_FOR_BEOWULF
...
!*** Calls from INPUT_DATA_FOR_BEOWULF:
!          READ_READ_SYSTEMY_DATA, READ_MODEL_DATA, SET_TRP, SET_FRQ,

 USE INPUT_DATA_FOR_BEOWULF
...
! Put arrays in body centred system.
 ...
 OPEN(NW1,FILE = 'Beowulf.mv1',STATUS = 'REPLACE')
 ...
 SELECT CASE (MXERR)
 CASE (0)
   WRITE(*,'(/T3,A)') 'Control file passed initial tests.  Computation begins.'
 CASE (1)
   WRITE(*,'(/T3,A//T3,A)') 'Computation begins.','Look at comments in BEOWULF.LOG'
 CASE(2)
   WRITE(*,'(/T3,A//T3,A//T3,A)') 'FATAL INPUT DATA ERRORS IN BEOWULF.CFL OR BEOWULF.INV', &
                                  'Refer to messages in BEOWULF.LOG', &
...
!  For time-domain, set up frequencies, interpolation times
   ...
   DO JS = LNTR(1,JL),LNTR(2,JL)
     DO JR = LNTR(3,JL),LNTR(4,JL)
       ...
       CALL NLSQ2 (JL,JRL,JSTAT,JS,JR,NPAR,NDATA,XDATA,RDATA,RWTS,PCTCNV,NDSTP,KPCT,CXPAR,ELAS,LBND,UBND, &
                   MAXITS,INVPRT,NW,NW1,SURVEY_TYPE,SOURCE_TYPE,NLINES,MLINES,LINE,HEADER_ID, &
 ...
 97 FORMAT(/T13,'Beowulf task started at ',I2.2,':',I2.2,' on',I3.2,1X,A,I5)
 98 FORMAT(//T3,'Beowulf task completed at ',I2.2,':',I2.2,' on',I3.2,1X,A,I5 &
           //T3,'Computation time = ',F10.2,' seconds.'//)
 99 FORMAT(T1,'/'/T1,'/ Beowulf task completed at ',I2.2,':',I2.2,' on',I3.2,1X,A,I5 &
...
!  XPAR   : transformed parameter array
 ...
 IF (NLYR > 1) THEN
   IF (FINAL) THEN
     ...
     END DO
     WRITE(NW,16) RES(NLYR),IMPORT(NLYR); WRITE(*,16) RES(NLYR),IMPORT(NLYR)
   ELSE
     WRITE(NW,3); WRITE(*,3)
     DO JL = 1,NLYR-1
 ...
 SUBROUTINE WRITE_INVDATA_TD (JL,JRL,NW,NW1,NCHNL,NLINES,MRXL,MCMP,SURVEY_TYPE,LINE,IDH,RX_TYPE, &
...
!   RWTS(I,J,K,L)   : inversion weights corresponding to RDATA
 ...
 IMPLICIT NONE
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80)
 INTEGER NW,NW1,NLINES,MRXL,MCMP,NCHNL,SURVEY_TYPE,JL,JRL,LC
 ...
 SELECT CASE (CMP(JL))
 CASE(1)
   WRITE(NW1,21) YXZPLT(1:3,JRL,JL), RDATA(1:NCHNL,JRL,1,JL)
   WRITE(NW1,22) YXZPLT(1:3,JRL,JL), BFTL(1:NCHNL,JRL,1,JL)
 CASE(2)                       
   WRITE(NW1,21) YXZPLT(1:3,JRL,JL), RDATA(1:NCHNL,JRL,2,JL)
   WRITE(NW1,22) YXZPLT(1:3,JRL,JL), BFTL(1:NCHNL,JRL,2,JL)
 CASE(3)                       
   WRITE(NW1,21) YXZPLT(1:3,JRL,JL), RDATA(1:NCHNL,JRL,3,JL)
   WRITE(NW1,22) YXZPLT(1:3,JRL,JL), BFTL(1:NCHNL,JRL,3,JL)
 CASE(12)                      
   WRITE(NW1,21) YXZPLT(1:3,JRL,JL), RDATA(1:NCHNL,JRL,1,JL), RDATA(1:NCHNL,JRL,2,JL)
   WRITE(NW1,22) YXZPLT(1:3,JRL,JL), BFTL(1:NCHNL,JRL,1,JL),  BFTL(1:NCHNL,JRL,2,JL)
 CASE(13)                      
   WRITE(NW1,21) YXZPLT(1:3,JRL,JL), RDATA(1:NCHNL,JRL,1,JL), RDATA(1:NCHNL,JRL,3,JL)
   WRITE(NW1,22) YXZPLT(1:3,JRL,JL), BFTL(1:NCHNL,JRL,1,JL),  BFTL(1:NCHNL,JRL,3,JL)
 CASE(23)                      
   WRITE(NW1,21) YXZPLT(1:3,JRL,JL), RDATA(1:NCHNL,JRL,2,JL), RDATA(1:NCHNL,JRL,3,JL)
   WRITE(NW1,22) YXZPLT(1:3,JRL,JL), BFTL(1:NCHNL,JRL,2,JL),  BFTL(1:NCHNL,JRL,3,JL)
 CASE(123)                     
   WRITE(NW1,21) YXZPLT(1:3,JRL,JL), RDATA(1:NCHNL,JRL,1,JL), RDATA(1:NCHNL,JRL,2,JL), RDATA(1:NCHNL,JRL,3,JL)
   WRITE(NW1,22) YXZPLT(1:3,JRL,JL), BFTL(1:NCHNL,JRL,1,JL),  BFTL(1:NCHNL,JRL,2,JL),  BFTL(1:NCHNL,JRL,3,JL)
 END SELECT
 ...
 21 FORMAT(/T3,'SURVEY_DATA',T15,2F12.1,F9.1,150G13.4)
 22 FORMAT(T3, 'MODEL_DATA', T15,2F12.1,F9.1,150G13.4)
 23 FORMAT(/T3,'Survey data:',T15,150G13.4)
 24 FORMAT(T3, 'Model data:', T15,150G13.4)
 25 FORMAT(T3,'Misfit (%):',F11.1,150F13.1)
...
!  Writes frequency-domain output in profile form.  IPPM = format control
 ...
 SUBROUTINE WRITE_INVDATA_FD (JL,JRL,NW,NW1,NFRQ,MCHNL,NLINES,MRXL,MCMP,LINE,RX_TYPE, &
...
!***     Calls:
!
!   NW, NW1         : output unit number for Beowulf.out
!   NFRQ            : number of frequencies
!   MCHNL           = NFRQ for Sampo;  = 2*NFRQ otherwise
!   NLINES          : number of lines
!   MRXL            : maximum number of receivers per line
...
!   RWTS(I,J,K,L)   : inversion weights corresponding to RDATA
 ...
 IMPLICIT NONE
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80)
 INTEGER NW,NW1,MCHNL,MCMP,NLINES,MRXL,NFRQ,ISYS,JL,JRL,IPPM,LC,KF1,KF2
 ...
 IF (IPPM == 1) THEN
   SELECT CASE (CMP(JL))
   CASE(1)
     WRITE(NW1,1) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,1,JL)
     WRITE(NW1,2) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,1,JL)
   CASE(2)        
     WRITE(NW1,1) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,2,JL)
     WRITE(NW1,2) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,2,JL)
   CASE(3)        
     WRITE(NW1,1) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,3,JL)
     WRITE(NW1,2) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,3,JL)
   CASE(12)       
     WRITE(NW1,1) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,1,JL), RDATA(1:MCHNL,JRL,2,JL)
     WRITE(NW1,2) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,1,JL),  BFTL(1:MCHNL,JRL,2,JL)
   CASE(13)       
     WRITE(NW1,1) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,1,JL), RDATA(1:MCHNL,JRL,3,JL)
     WRITE(NW1,2) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,1,JL),  BFTL(1:MCHNL,JRL,3,JL)
   CASE(23)       
     WRITE(NW1,1) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,2,JL), RDATA(1:MCHNL,JRL,3,JL)
     WRITE(NW1,2) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,2,JL),  BFTL(1:MCHNL,JRL,3,JL)
   CASE(123)      
     WRITE(NW1,1) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,1,JL), RDATA(1:MCHNL,JRL,2,JL), RDATA(1:MCHNL,JRL,3,JL)
     WRITE(NW1,2) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,1,JL),  BFTL(1:MCHNL,JRL,2,JL),  BFTL(1:MCHNL,JRL,3,JL)
   END SELECT
 ELSE
   SELECT CASE (CMP(JL))
   CASE(1)
     WRITE(NW1,11) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,1,JL)
     WRITE(NW1,12) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,1,JL)
   CASE(2)        
     WRITE(NW1,11) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,2,JL)
     WRITE(NW1,12) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,2,JL)
   CASE(3)        
     WRITE(NW1,11) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,3,JL)
     WRITE(NW1,12) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,3,JL)
   CASE(12)       
     WRITE(NW1,11) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,1,JL), RDATA(1:MCHNL,JRL,2,JL)
     WRITE(NW1,12) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,1,JL),  BFTL(1:MCHNL,JRL,2,JL)
   CASE(13)       
     WRITE(NW1,11) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,1,JL), RDATA(1:MCHNL,JRL,3,JL)
     WRITE(NW1,12) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,1,JL),  BFTL(1:MCHNL,JRL,3,JL)
   CASE(23)       
     WRITE(NW1,11) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,2,JL), RDATA(1:MCHNL,JRL,3,JL)
     WRITE(NW1,12) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,2,JL),  BFTL(1:MCHNL,JRL,3,JL)
   CASE(123)      
     WRITE(NW1,11) YXZPLT(1:3,JRL,JL), RDATA(1:MCHNL,JRL,1,JL), RDATA(1:MCHNL,JRL,2,JL), RDATA(1:MCHNL,JRL,3,JL)
     WRITE(NW1,12) YXZPLT(1:3,JRL,JL),  BFTL(1:MCHNL,JRL,1,JL),  BFTL(1:MCHNL,JRL,2,JL),  BFTL(1:MCHNL,JRL,3,JL)
   END SELECT
 END IF
 ...
 END IF
  ...
  2 FORMAT(T3, 'MODEL_DATA', T15,2F12.1,F9.1,500F3.4)
...
!
 ...
 INTEGER JL,JRL,JSTAT,JS,JR,NPAR,NDATA,MAXITS,ITS,PITS,ICNT,NW,NW1,INVPRT,CXPAR(NPAR),NSV,NLINES,MLINES, &
...
!  size in error.
 ...
 CPLT = PLT_PT(IPLT(JL))
 CALL GET_UNITS_TEXT (UNITS(JL),UTXT)
 WRITE(NW1,18) JSTAT,TRIM (ADJUSTL (CLINE)),HEADER_ID(JL),KDEG,TRIM(UTXT(2)),TRIM (ADJUSTL (CPLT))
 ...
 ITER_LOOP: DO ITS = 1, MAXITS
   ...
   IF (ITS == 1) THEN
     ...
     IF (INVPRT == 3) THEN
       WRITE(NW,9)
       IF (TDFD == 1) THEN
         CALL WRITE_INVDATA_TD (JL,JRL,NW,NW1,NCHNL,NLINES,MRXL,MCMP,SURVEY_TYPE, &
                                LINE,IDH,RX_TYPE,YXZPLT,CMP,RMODL,RDATA,RWTS)
       ELSE
         CALL WRITE_INVDATA_FD (JL,JRL,NW,NW1,NFRQ,MCHNL,NLINES,MRXL,MCMP,LINE,RX_TYPE, &
...
!  Test MAXITS, and reduction with respect to PDRE, PCTCNV.
   ...
   IF (ITS > 10) THEN
     MOVE = .TRUE.
     IF (DELT + DELTOLD < 0.01) THEN
       WRITE(NW,14);  WRITE(*,14)
     ELSE IF (DELT < 0.0001* FOLD) THEN
...
!*******************************************
   ...
   OPEN(NW1,FILE = 'Beowulf.mv1',STATUS = 'OLD', POSITION = 'APPEND')
 ...
 11 FORMAT(/T3,'Data and Misfit Final - (East, North, Elevation) =',2F9.1,F7.1)
 12 FORMAT(//T3,50('='))
 14 FORMAT(/T3,'Error reduction after last 2 iterations < 0.01 percent.')
 ...
 23 FORMAT(/T3,'Error reduction < .01 percent of RMS error.')
 ...
 32 FORMAT(T1,'/'/T1,'/ FINAL_ITERATION  ',I2.2/T1,'/ PERCENT_RMS_ERROR:',F8.2,3X,'RSVT:',F7.3)
 33 FORMAT(/T1,'/ FINAL_MODEL',84F13.2)
 34 FORMAT(T1,'/ IMPORTANCE ',84F13.2)
 35 FORMaT(I7,I9,2F11.1,F8.1,I6,F8.2,60G13.4)
